...
**支持平台：** 抖音、小红书、YouTube、Bilibili、TikTok、网易云、推特、Instagram 等其他平台  
...
- **TikTok**：博主订阅
- **小红书**：博主订阅
- **网易云**：歌单订阅（基于歌曲 ID 检测更新）
- **Instagram**：博主订阅（基于 instagrapi）
...
- 支持直接发送链接下载视频
- 支持添加网易云歌单订阅（`/sub <网易云歌单链接>`）
- 支持网易云歌曲搜索下载（`/music <关键词>`）
...
- 支持录制质量设置和存储管理
- 自动检测直播结束并保存录制文件
- **AI智能高光分析**：录制结束后，系统可基于弹幕热度和情感分析自动提取高光时刻进行切片，并接入大语言模型（如 DeepSeek）智能生成高能标题与剧情摘要
- **一键导出资源包**：将所有高光切片视频、对应的独立弹幕数据（JSON）与剧情文案自动按片段封装为独立子目录，打包为 ZIP 供一键下载，完美适配短视频二次剪辑需求
