        ...
        if text.startswith('/'):
            ...
            if command == '/start':
                ...
                await self.send_message(chat_id,
                    ...
                    "• **直播间** → 引导添加录制监控\n\n"
                    "📥 **媒体转存 · LIFETIME**\n"
                    "直接发送 **视频/图片/文件** 给机器人，自动转存到本地（可点击按钮取消）。\n"
                    "内置 MTProto 协议突破 20MB 限制，最高支持约 **2GB** 直连下载。\n"
                    "⚠️ LIFETIME 授权专享，仅白名单 Chat ID 可用。\n\n"
                    "🔔 **下载通知交互**\n"
                    "下载完成/失败后 Bot 会推送消息，支持点击按钮操作：\n"
                    "• ✅ 完成通知 → **「发送到Bot」** 将文件发回聊天直接查看\n"
                    "• ❌ 失败通知 → **「重试下载」** 或 **「删除任务」**\n\n"
                    "1️⃣ **强制指令** (识别不准确时使用)\n"
                    "• `/sub <链接>` — 强制添加为订阅源\n"
                    "• `/live <链接>` — 强制添加为直播监控\n"
                    ...
                    "2️⃣ **交互式管理命令** (支持按钮操作)\n"
                    "• `/status` — 系统状态与统计概览\n"
                    "• `/failed` — 失败任务列表（可重试/删除详情）\n"
                    "• `/subs` — 视频订阅列表（翻页 · 筛选 · 查看详情 · 批量下载 · 设置频率）\n"
                    "• `/lives` — 直播订阅列表（翻页 · 详情 · 开关录制 · 手动控制）\n\n"
                    "3️⃣ **音乐搜索下载**\n"
                    ...
                    "• `/id` — 获取当前 Chat ID\n\n"
                    "💡 **订阅管理交互**\n"
                    "进入订阅详情后支持：| 更新检测 | 视频列表 | 全量同步 | 设置 |\n"
                    "视频列表支持：| 📥 一键批量下载 | 🔁 重试失败 |\n"
                    "设置面板支持：| 切换自动下载 | 调节检测频率 | 删除订阅 |\n\n"
                    "📪 **反馈与建议**\n"
