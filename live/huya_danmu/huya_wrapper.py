...
class HuyaDanmuNodeProcess:
    """管理Huya弹幕Node.js子进程的封装类"""
    ...
    def __init__(
        ...
        self._script_path = os.path.join(self._script_dir, "huya_danmu_collector.js")
    ...
    def start(self):
        ...
        logger.info(
            f"[HuyaDanmu] 启动Node.js弹幕进程: room={self.room_id}, output={self.output_path}"
        )
        ...
        try:
            ...
            self._thread = threading.Thread(
                ...
                name=f"huya-danmu-{self.anchor_name}",
                daemon=True,
            )
            self._thread.start()

        except Exception as e:
            logger.error(f"[HuyaDanmu] 启动Node进程失败: {e}")
            self._running = False
            raise
        ...
        try:
            for line in self._process.stdout:
                ...
                try:
                    ...
                    if msg_type == "log":
                        logger.info(f"[HuyaDanmu] Room {msg.get('roomId')}: {msg.get('msg')}")
                    elif msg_type == "status":
                        logger.info(f"[HuyaDanmu] Status: {msg}")
                    ...
                    # 非JSON格式，直接输出
                    logger.debug(f"[HuyaDanmu] Node输出: {line.strip()}")

        except Exception as e:
            logger.warning(f"[HuyaDanmu] stdout读取异常: {e}")
    ...
    def _publish_danmu(self, event: dict):
        ...
        try:
            ...
            logger.debug(f"[HuyaDanmu] 推送弹幕失败: {e}")
    ...
    def stop(self):
        ...
        logger.info(f"[HuyaDanmu] 停止Node.js弹幕进程: room={self.room_id}")
        ...
        try:
            ...
            logger.warning(f"[HuyaDanmu] 停止Node进程时出错: {e}")
    ...
    def send_command(self, cmd: str):
        ...
        try:
            ...
            logger.warning(f"[HuyaDanmu] 发送命令失败: {e}")
